...
const filePath = 'C:/NODEJS/aulas/rede/img/file_300_250.jpg'
    ...
    resultado.forEach(async (convenio) =>{
        ...
        addToFormData(formData, 'start_date',Date().toString)
        if(online){
          addToFormData(formData, 'title','onLine')
        }else{
          addToFormData(formData, 'title','local')
        }
        if (desconto_em_folha) {
        ...
        const [resultadoEsp] = await sequelizeGuia.query(queryEspecialidades);
                ...
                resultadoEsp.forEach(async(areaEspecialidade) => {
                    ...
                    categorias.push(descricao_area)
                    //console.log(descricao_area + '-' + id_rede)
                   }
                )
          //console.log('Catagorias no Array: ' + categorias)

          addToFormData(formData, 'categories',categorias.toString)
            ...
            resultadoEnd.forEach(async (enderecoConv)=>{
               ...
               //console.log(endereco + '-' +  numero + '-' +  complemento + '-' +  bairro )
                ...
                enderecoFormatado = id_gds + ' ==> ' +  endereco  + ',' +  numero + ',' + bairro  + ',' +  Nm_cidade + '-CEP:' + cep  + ' Fone: ' 
